...
const SubstitutePage: React.FC<SubstitutePageProps> = ({ user }) => {
  ...
  const handleCreate = async () => {
    if (!formData.absentTeacherId || !formData.className) return alert('Thiếu thông tin!');
    ...
    <div className="space-y-6">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl max-w-lg w-full p-10">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={formData.absentTeacherId} onChange={e => setFormData({...formData, absentTeacherId: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-bold">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Ngày</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-2">Lớp</label>
                  <input type="text" value={formData.className} onChange={e => setFormData({...formData, className: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm font-bold" placeholder="VD: 9/1" />
                </div>
                <div>
