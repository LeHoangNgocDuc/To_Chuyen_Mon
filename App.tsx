...
const App: React.FC = () => {
  ...
  const handleApproveUser = async (userToApprove: User) => {
    ...
    try {
      ...
      setUsers(users.map(u => u.id === userToApprove.id ? updatedUser : u));
      alert(`Đã phê duyệt thành viên: ${userToApprove.name}`);
    } catch (e) {
      alert('Lỗi phê duyệt!');
    }
  };
                ...
                <div className="p-5 bg-blue-50/50 rounded-[2rem] border-2 border-blue-100 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {assignedClasses.map(item => (
                      <span key={item} className="px-3 py-1 bg-white border border-blue-100 text-blue-600 rounded-xl text-[10px] font-black">{item}</span>
                    ))}
                  </div>
                </div>
  ...
  return (
    <Layout user={currentUser} activeTab={activeTab} setActiveTab={setActiveTab} onLogout={() => setCurrentUser(null)} currentYear={currentYear} setCurrentYear={setCurrentYear}>
      {activeTab === 'dashboard' && <Dashboard user={currentUser} year={currentYear} notifications={notifications} onRefresh={fetchData} onUpdateProfile={handleUpdateProfile} />}
      {activeTab === 'schedule' && <SchedulePage user={currentUser} />}
      {activeTab === 'assignment' && <AssignmentPage user={currentUser} users={users} onApprove={handleApproveUser} onChangeRole={handleChangeUserRole} onDeleteUser={fetchData} />}
